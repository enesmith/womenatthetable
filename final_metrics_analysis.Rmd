...
In-processing mitigates bias in classifiers while building a model. The in-processing technique we use is the prejudice remover algorithm from AIF360. The prejudice remover is a method for reducing indirect prejudice (i.e. how the COMPAS algorithm is racially biased because it uses proxy variables for race). This approach implements two different regularizers, one to avoid overfitting and one to enforce fair classification ( [Kamishima et. al.](https://link.springer.com/content/pdf/10.1007/978-3-642-33486-3_3.pdf)). The prejudice remover regularizer works by minimizing the prejudice index, a mathematical equation for quantifying fairness defined by [Kamishima et. al](https://link.springer.com/content/pdf/10.1007/978-3-642-33486-3_3.pdf). This in turn enforces a classifier’s independence from sensitive information (like race). 
